...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    @Query("SELECT new ru.practicum.ewm.model.WatchStats(h.app, h.uri, COUNT(h.ip)) " +
            "FROM EndpointHit h " +
            "WHERE h.uri = ?1 " +
    ...
    @Query("SELECT new ru.practicum.ewm.model.WatchStats(h.app, h.uri, COUNT(DISTINCT h.ip)) " +
            "FROM EndpointHit h " +
            "WHERE h.uri = ?1 " +
            "AND (h.timestamp BETWEEN ?2 AND ?3) " +
            "GROUP BY h.app, h.uri, h.ip")
