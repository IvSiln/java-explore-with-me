...
public class RequestServiceImpl implements RequestService {
    ...
    private ParticipationRequest getRequestIfExists(Long requestId) {
        return requestRepository.findById(requestId)
                .orElseThrow(() -> new NotFoundException(PARTICIPATION_REQUEST_NOT_FOUND));
    }
    ...
    private void checkIfRequestExists(Long userId, Long eventId) {
        if (requestRepository.findFirst1ByEventIdAndRequesterId(eventId, userId).isPresent()) {
            throw new ConflictException(REQUEST_ALREADY_EXIST);
        }
    }
    ...
    private void checkUserIsInitiator(Long userId, Event event) {
        ...
        if (userId.equals(initiatorId)) {
            throw new ConflictException(OWNER_NOT_ALLOWED_TO_ADD_REQUEST);
        }
    }
